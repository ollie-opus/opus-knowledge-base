---
icon: lucide/mouse-pointer
search:
  exclude: true
---

# Interacting with a task
<span data-uuid="4e951955-763b-426b-b87e-236cf24ec19b" style="display:none"></span>

e.g.

- How to interact with reminder task (e.g. checklist reminder)
- Interacting with fields
- Commenting
- Sub tasks
- Snoozing Tasks
- Resolving Tasks

## Reminder Tasks
<span data-uuid="53327301-5943-46a9-91bc-78f8e3d43dda" style="display:none"></span>

Reminder tasks are automatically generated by the system if a requirement becomes due. 

For example, it could be a task to complete an e-learning, provide a document or fill in a checklist.

<span data-uuid="418fee33-4807-45a7-8f71-924120a662ae" style="display:none"></span>
![](../assets/media/occ-captures/todos/uuid/system-message-light-mode.png#only-light){ style="border-radius: 8px" width="400" loading=lazy }
![](../assets/media/occ-captures/todos/uuid/system-message-dark-mode.png#only-dark){ style="border-radius: 8px" width="400" loading=lazy }

## Interacting with fields
<span data-uuid="b833aaf9-7e2c-44f5-ace4-5457b7cc3a1a" style="display:none"></span>

## Commenting
<span data-uuid="4b8e4a57-882d-4316-bace-27b350ba5491" style="display:none"></span>

## Creating sub tasks
<span data-uuid="086aa064-7bf0-4607-8197-419524b11e9d" style="display:none"></span>

## Snoozing tasks
<span data-uuid="991647ca-b013-4539-b8bf-382edc17dca3" style="display:none"></span>

## Resolving tasks
<span data-uuid="362638da-9cc6-4327-8c5c-13cfde0f761a" style="display:none"></span>
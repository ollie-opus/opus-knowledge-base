---
icon: lucide/layout-dashboard
search:
  exclude: true
---

# Overview
<span data-uuid="f00a359d-b11b-4a7c-8672-4344f4618552" style="display:none"></span>

## What is Opus Compliance Cloud?
<span data-uuid="83ad77b8-b137-43ca-a4ed-06ec4905ced0" style="display:none"></span>

Opus Compliance Cloud is a compliance management system focused on Health & Safety. It enables you to manage your sites, employees, assets, contractors, and more - all in one place - helping your organisation stay on top of its compliance responsibilities.

## Get started
<span data-uuid="b8e1850f-82f0-41f1-89fd-5ff9717ff265" style="display:none"></span>

Browse the guides using the search bar at the top or the navigation bar on the left.


<span class="mb-label mb-label-emerald">If you're new to Opus Compliance Cloud</span> - we recommend starting with a tailored guidebook below that best suits your needs.

## Guidebooks
<span data-uuid="d55541b1-6c53-4664-bf44-a6bc5d5e4823" style="display:none"></span>


<span data-uuid="a99c20d5-4717-42a3-bc29-bd5e0eef47a0" style="display:none"></span>

<div class="grid" markdown>

<div class="card" markdown>

<span data-uuid="c01bd4cc-b706-448b-83ea-932ad8dc462d" style="display:none"></span>
<span class="mb-label mb-label-blue">:lucide-user-shield: Using Opus Compliance Cloud</span>

---

Designed for those with **User access** on Opus Compliance Cloud.

<span data-uuid="0a43857c-4a84-44c7-a692-48d1594e3e78" style="display:none"></span>
[Using Opus Compliance Cloud :lucide-arrow-up-right:](using-opus-compliance-cloud.md){ .md-button .custom-button-blue .custom-button--borderless .custom-button--slim }

</div>

<div class="card" markdown>

<span data-uuid="f467ada2-d3d2-42b0-9907-7fa819dec6a6" style="display:none"></span>

</div>

<div class="card" markdown>

<span data-uuid="b310583f-2734-4da3-872c-419c1e6eff8b" style="display:none"></span>

</div>

<div class="card" markdown>

<span data-uuid="10114067-d639-415c-a993-2a78507a3506" style="display:none"></span>

</div>

</div>